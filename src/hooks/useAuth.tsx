...
function AuthProvider({ children }: AuthProviderData) {
  ...
  async function signIn() {
    try {
      ...
      const REDIRECT_URI = 'https://auth.expo.io/@brunoambr/streamData';
      ...
      const authResponse = await startAsync({ authUrl }) as AuthSessionResult;
      ...
      if (authResponse.type === 'success' && authResponse.params.error !== 'access_denied') {
        ...
        const { id, email, display_name, profile_image_url } = userResponse.data.data[0];
